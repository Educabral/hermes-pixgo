...
**Gmail App Password issue:** The 16-char app password often contains spaces (e.g. `jsyj kshw ould yull`). Store it in a script file (e.g. `echo -n "password with spaces"`) and reference via `backend.auth.cmd`, NOT inline — the `raw` key in config.toml may not handle spaces correctly.

**Port choice:** Gmail SMTP works on both 465 (TLS, used in this session) and 587 (STARTTLS). Use 465 with `tls` encryption for simpler setup; use 587 with `start-tls` if 465 is blocked.
